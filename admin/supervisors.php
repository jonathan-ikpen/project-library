    ...
    if ($action === 'create_supervisor') {
        ...
        if (empty($name) || empty($email) || empty($password)) {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $pdo->prepare("INSERT INTO users (name, email, password_hash, role) VALUES (?, ?, ?, 'supervisor')");
...
$stmt = $pdo->query("SELECT id, name, email, status, created_at FROM users WHERE role = 'supervisor' ORDER BY created_at DESC");
$users = $stmt->fetchAll();
...
<div class="mb-4">
    ...
    <p class="subtext">View existing supervisors and provision new supervisor accounts.</p>
</div>
...
<div class="grid-auto-fit mb-4">
    <div class="panel">
        <h3 class="mb-3">Create Supervisor</h3>
...
<div class="panel">
    <h3 class="mb-3">All Supervisors</h3>
    <?php if (empty($users)): ?>
        <?= render_empty_state("No Supervisors Found", "There are currently no supervisor accounts.") ?>
    <?php else: ?>
