...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate inputs
    if (empty($title) || empty($abstract) || !$supervisor_id || !$category_id) {
        ...
        try {
            ...
            // 1. Insert Project
            $stmt = $pdo->prepare("
                INSERT INTO projects (title, abstract, student_id, supervisor_id, category_id, year, language) 
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$title, $abstract, $_SESSION['user_id'], $supervisor_id, $category_id, $year, $language]);
            ...
            $pdo->commit();
            $success = 'Project uploaded successfully and is now pending supervisor approval.';
...
<div class="mb-4">
    ...
    <p class="subtext">Submit your final project files and metadata for supervisor review.</p>
</div>
...
<?php else: ?>
    <div class="panel" style="max-width: 800px;">
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <button type="submit" style="width: 100%;">Submit Project</button>
        </form>
    </div>
<?php endif; ?>
